defmodule Day01 do
	...
	def part2 do
		input_stream = Stream.cycle(input() )
		stream = Stream.scan(input_stream , &+/2)
		Enum.reduce_while(stream, MapSet.new(), &seen?/2)
	end

	defp seen?(e, acc) do
		case MapSet.member?(acc, e) do
			true -> {:halt, e}
			false -> {:cont, MapSet.put(acc, e)}
		end
	end
end
